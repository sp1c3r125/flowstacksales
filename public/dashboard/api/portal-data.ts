...
// ─────────────────────────────────────────────────────────────
// Token registry — maps client tokens to tenant IDs
// In production you'd store this in Airtable or an env var JSON blob.
// For now: set PORTAL_TOKENS as a JSON env var in Vercel:
//   PORTAL_TOKENS={"demo-client":"demo","palawantours_ppc":"palawantours_ppc"}
// ─────────────────────────────────────────────────────────────
function resolveTokenRegistry(): Record<string, string> {
  ...
  // Fallback: demo token always works
...
function computeStats(leads: any[]) {
  ...
  const week   = leads.filter(l =>
    l.created && (Date.now() - new Date(l.created).getTime()) < 7 * 86400000
  ).length;
  const qual   = leads.filter(l => l.status === "Qualified").length;
  const conv   = total > 0 ? Math.round((qual / total) * 100) : 0;
  const action = leads.filter(l =>
    /follow|review|pending/i.test(l.next_action || "")
  ).length;
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  // CORS — portal HTML is same origin on Vercel, but allow explicit for local dev
  ...
  // Extract token — support both Authorization header and ?token= query param
  ...
  if (!pat || !baseId) {
    // Return demo data instead of erroring — portal still renders
    return res.status(200).json({
      ok:     true,
      mode:   "demo",
      tenant: tenantId,
      stats:  { total: 6, week: 6, velDelta: 12, conv: 84, action: 3, hours: 128, tasks: 24, channels: { Facebook: 2, Website: 2, Instagram: 1, Referral: 1 } },
      leads:  [],
      activities: [],
      workflows: [
