...
export const ProposalView: React.FC<Props> = ({ appState, onReset }) => {
  ...
  const handleDeploy = async () => {
    ...
    try {
      ...
      addLog(`> IMPLEMENTATION PLAN: ${activeStack.name.toUpperCase()}`);
      await delay(700);
      ...
      addLog('> IMPLEMENTATION PLAN DEPLOYMENT: CONFIRMED.');
      await delay(400);
    ...
    <div className="space-y-6 animate-[fadeIn_1s_ease-out] pb-20 relative">
      ...
      <div className="text-center mb-10 max-w-3xl mx-auto">
        <h1 className="text-4xl md:text-5xl font-bold text-white mb-4 tracking-tight leading-tight">
          Find the Revenue Leaks in Your <span className="text-emerald-500">Follow-Up</span> (In 24 Hours)
        </h1>
        <p className="text-slate-400 text-lg mb-8">
          Upload your data below for a free, automated analysis of missed leads and lost revenue.
        </p>

        {!loading && deployStatus !== 'success' && (
          <div className="flex justify-center mb-12">
            <Button
              onClick={handleDeploy}
              disabled={loading || deployStatus === 'sending'}
              className={`px-10 py-8 text-xl font-bold shadow-2xl hover:scale-105 transition-transform ${isHighValue ? 'bg-red-600 hover:bg-red-500 border-red-500' : 'bg-emerald-600 hover:bg-emerald-500 border-emerald-500'}`}
            >
              {deployStatus === 'sending' ? (
                <><Wifi className="mr-2 h-6 w-6 animate-pulse" /> CONNECTING...</>
              ) : (
                <><CheckCircle className="mr-2 h-6 w-6" /> GET A FREE LEAKAGE AUDIT</>
              )}
            </Button>
          </div>
        )}
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <div className="text-xs text-emerald-600/70">
                    Implementation Plan credentials sent to {appState.ingest?.contactEmail || 'N/A'}
                  </div>
                </div>
              </div>
