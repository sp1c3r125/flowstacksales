...
export const ProposalView: React.FC<Props> = ({ appState, onReset }) => {
  ...
  const proposalSections = useMemo(() => groupNarrativeBlocksByHeading(visibleProposalBlocks), [visibleProposalBlocks]);
  const executiveSummaryBlocks = proposalSections.get('Executive Summary') || [];
  const proposalBodyBlocks = visibleProposalBlocks.filter((block, index) => {
    if (block.type === 'heading' && normalizeSectionLabel(block.text) === 'Executive Summary') {
      return false;
    }

    let currentHeading = '';
    for (let i = index; i >= 0; i -= 1) {
      const candidate = visibleProposalBlocks[i];
      if (candidate?.type === 'heading') {
        currentHeading = normalizeSectionLabel(candidate.text);
        break;
      }
    }

    return currentHeading !== 'Executive Summary';
  });
...
function normalizeSectionLabel(value: string) {
  return normalizePdfSafeText(value).replace(/:+$/g, '').trim();
}

function groupNarrativeBlocksByHeading(blocks: NarrativeBlock[]) {
  const sections = new Map<string, NarrativeBlock[]>();
  let currentHeading = 'Executive Summary';

  blocks.forEach((block) => {
    if (block.type === 'heading') {
      currentHeading = normalizeSectionLabel(block.text) || 'Executive Summary';
    ...
    <div className="space-y-6 animate-[fadeIn_1s_ease-out] pb-20 relative">
      ...
      <BentoGrid>
        ...
        <BentoCard
          ...
          headerAction={<ArrowRight size={14} className="text-cyan-200/65" />}
        >
          <NeonPanel className="flex-1 max-h-[60vh] overflow-y-auto p-8">
            {loading ? (
              <div className="flex flex-col items-center justify-center h-full space-y-6">
                <div className="w-16 h-16 border-4 border-blue-400/15 border-t-emerald-500 rounded-full animate-spin"></div>
                <div className="space-y-2 text-center">
                  <p className="text-emerald-400 font-mono text-sm animate-pulse">Generating your live proposal summary...</p>
                  <p className="text-cyan-100/55 text-xs">Building the client-facing recommendation from your submitted inputs.</p>
                </div>
              </div>
            ) : (
              <article className="max-w-none space-y-6">
                <div className="rounded-2xl border border-cyan-400/16 bg-[linear-gradient(135deg,rgba(8,24,47,0.92),rgba(7,20,39,0.88),rgba(3,11,29,0.96))] p-6 shadow-[0_0_0_1px_rgba(56,189,248,0.03)]">
                  <div className="mb-3 flex items-center gap-2">
                    <div className="h-2.5 w-2.5 rounded-full bg-emerald-400" />
                    <span className="text-[11px] uppercase tracking-[0.22em] text-cyan-100/65">Live proposal summary</span>
                  </div>
                  {executiveSummaryBlocks.length ? (
                    <div className="space-y-4">
                      {executiveSummaryBlocks.map((block, index) => {
                        if (block.type === 'bullets') {
                          return (
                            <ul
                              key={`executive-bullets-${index}`}
                              className="space-y-3 rounded-xl border border-cyan-400/14 bg-[#04101f]/80 p-5"
                            >
                              {block.items.map((item, itemIndex) => (
                                <li
                                  key={`executive-bullet-${index}-${itemIndex}`}
                        ...
                        return (
                          <p
                            key={`executive-paragraph-${index}`}
                            className="text-[15px] leading-8 text-slate-100 whitespace-pre-line"
                          >
                            {block.text}
                          </p>
                        );
                    ...
                    <p className="text-[15px] leading-8 text-slate-100 whitespace-pre-line">
                      {streamingText || proposal || 'Generating executive summary...'}
                    </p>
                  )}
                </div>

                <div className="space-y-5">
                  {proposalBodyBlocks.map((block, index) => {
                    if (block.type === 'heading') {
                      const isBusinessContext = normalizeSectionLabel(block.text) === 'Operational Signals';
                      return (
                        <div key={`heading-${index}`} className="pt-2 first:pt-0">
                          <h2 className="text-lg font-bold uppercase tracking-[0.14em] text-emerald-400">
                            {isBusinessContext ? 'Business Context' : block.text}
                          </h2>
                        </div>
                      );
                    }
                    ...
                    if (block.type === 'bullets') {
                      return (
                        <ul
                          key={`bullets-${index}`}
                          className="space-y-3 rounded-xl border border-blue-400/12 bg-[linear-gradient(135deg,rgba(7,20,39,0.82),rgba(3,11,29,0.92))] p-5"
                        >
                          {block.items.map((item, itemIndex) => (
                            <li
